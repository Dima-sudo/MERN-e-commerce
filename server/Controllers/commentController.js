...
exports.create = async (req, res) => {
  ...
  try {
    ...
    if (!foundProduct)
      return res
        .status(200)
        .json({ message: "No item found", status: "success" });
...
exports.delete = async (req, res) => {
  ...
  try {
    ...
    if (!foundProduct)
      return res
        .status(200)
        .json({ message: "No product found", status: "success" });
    if (!deletedComment)
      return res
        .status(200)
        .json({ message: "No comment found", status: "success" });
    ...
    return res
      .status(500)
      .json({
        message: "Error during comment deletion",
        error: err,
        status: "failure",
      });
  }
...
exports.findById = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({
        message: "Error finding the comment",
        error: err,
        status: "failure",
      });
  }
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const updatedComment = {
      ...req.body,
      created: foundComment.created,
      createdBy: foundComment.createdBy,
    };
