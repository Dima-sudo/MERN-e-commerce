...
exports.validate_Account_Creation_Form = async (req, res, next) => {
  ...
  if(!email || !password || !confirmPassword) return res.status(406).json({message: "Your form has missing information", error: "Missing information", status: "failure"})
  ...
  if(email.length < 4) return res.status(400).json({message: "Email has to be atleast 5 characters long", error: "Email too short", status: "failure"})
  if(!email.includes('@') || !email.includes('.') || email.includes(' ')) return res.status(400).json({message: "The email or format entered is invalid", error: "Invalid email", status: "failure"});
  if(password.length < 5) return res.status(400).json({message: "Password has to be atleast 6 characters long", error: "Password too short", status: "failure"})
...
exports.validate_Laptop_Creation_Form = async (req, res, next) => {
    ...
    if (!form.screen.includes(":"))
      return res
        ...
        .json({
          message:
            "Screen field must include resolution in WidthxHeight format, i.e 1920:1080.",
  ...
  if(req.files){
    ...
    for(let key in req.files){
      ...
      if(req.files[key].size > 6291456){
        return res.status(400).json({message: "The file provided is larger than 6 MB, product creation failed", error: 'File size exceeds limit', status: "failure"});
      }
      //
     ...
     if(fileList.length > 3){
      return res.status(500).json({message: "More than 3 files provided, product creation failed", error: 'Too many files', status: "failure"});
    }
    //
